...
app.use(session({
    secret: 'secret_for_server_js', // Replace with a secure secret
    resave: false,
...
// Example: server code to handle checking agent request status
app.get('/check-request-status', (req, res) => {
    const email = req.session.user.email;
...
// Endpoint to fetch messages for a specific request_id
app.get('/fetchMessagesAgent/:requestId', (req, res) => {
    const requestId = req.params.requestId;
    const email = req.session.user.email; // Retrieve email from session

    // Query to select messages where fetched is FALSE
    const selectQuery = `
        SELECT * FROM Messages 
        WHERE Request_id = ? AND fetched = FALSE
    `;

    pool.query(selectQuery, [requestId], (err, results) => {
        if (err) {
            console.error('Error fetching messages:', err);
            return res.status(500).json({ success: false, message: 'Internal server error' });
        }

        // If messages are found, update their fetched status
        if (results.length > 0) {
            const messageIds = results.map(message => message.Message_id);
            const updateQuery = `
                UPDATE Messages 
                SET fetched = TRUE 
                WHERE Message_id IN (?)
            ...
            pool.query(updateQuery, [messageIds], (err) => {
                if (err) {
                    console.error('Error updating fetched status:', err);
                    return res.status(500).json({ success: false, message: 'Error updating fetched status' });
                }

                // Send the fetched messages back to the client
                res.json({ success: true, messages: results, userEmail: email });
            });
        } else {
            // No new messages to fetch
            res.json({ success: true, messages: [], userEmail: email });
        }
    });
});


app.get('/fetchMessagesUser/:requestId', (req, res) => {
    const requestId = req.params.requestId;
    const email = req.session.user.email; // Retrieve email from session
...
app.get('/api/agent', (req, res) => {
    if (req.session.user) {  // Changed from user to agent
        const { name, surname, email, phone, address, profileImage } = req.session.user;
        ...
        res.json({ 
            success: true, 
            user: {  // Changed from user to agent
                name, 
                surname, 
                email, 
                phone, 
        ...
        res.status(401).json({ success: false, message: 'Agent not logged in.' }); // Adjusted error message for agent
    }
});
...
app.post('/register', (req, res) => {
    ...
    pool.query(checkEmailQuery, [email], (err, results) => {
        ...
        if (results.length > 0) {
            ...
            return res.json({ success: false, message: 'Email already exists. Please use a unique one.' });
        } 

        // If email doesn't exist, proceed with hashing the password and inserting into the database
        bcrypt.hash(password, 10, (err, hashedPassword) => {
            ...
            const query = 'INSERT INTO Sign_Up (firstName, lastName, email, phone, address, password, userType) VALUES (?, ?, ?, ?, ?, ?, ?)';
